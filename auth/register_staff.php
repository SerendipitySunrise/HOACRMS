...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!registrationInvitationValid($formData['invitation_code'], STAFF_REGISTRATION_ACCESS_KEY)) {
        ...
        $messageType = 'error';
    } elseif ($formData['department_id'] <= 0 || $formData['staff_role'] === '') {
        $message = 'Please select a department and enter your staff role.';
        $messageType = 'error';
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            if (!mysqli_stmt_execute($stmt)) {
                ...
                $staffStmt = mysqli_prepare(
                    $conn,
                    'INSERT INTO staff (UserID, DepartmentID, StaffRole, Specialization)
                     VALUES (?, ?, ?, ?)'
                );
                $spec = $formData['specialization'] !== '' ? $formData['specialization'] : null;
                mysqli_stmt_bind_param($staffStmt, 'iiss', $userId, $formData['department_id'], $formData['staff_role'], $spec);

                if (mysqli_stmt_execute($staffStmt)) {
                    mysqli_commit($conn);
                    header('Location: login.php?portal=staff&registered=success');
                    exit();
                }
...
<head>
    ...
    <title>Staff Registration - CarePath</title>
    ...
    <div class="main-container">
        ...
        <?php if (count($departments) === 0): ?>
            <div class="no-departments">
                ...
                <p>No departments found. Add rows in the <code>departments</code> table before registering staff.</p>
            </div>
        <?php else: ?>
            <form class="form" action="register_staff.php" method="POST">
                <!-- Invitation Code -->
                ...
                <div class="form-group">
                    ...
                    <div class="input-wrapper">
                        <span class="input-icon-left"><i class="fas fa-user-tag"></i></span>
                        <input 
                            type="text" 
                            id="staff_role" 
                            name="staff_role" 
