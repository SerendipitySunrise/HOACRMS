...
mysqli_stmt_bind_param($queueStmt, 'is', $patient['PatientID'], $today);
mysqli_stmt_execute($queueStmt);
$queueResult = mysqli_stmt_get_result($queueStmt);
$queue = mysqli_fetch_assoc($queueResult);

// Get current serving number for today (if patient has a queue)
$nowServing = null;
if ($queue) {
    ...
    mysqli_stmt_bind_param($servingStmt, 's', $today);
